--// free Key system by me

--== CONFIGURATION ==--
local ownerKey = "defyowner123" -- 🔑 Owner master key
local linkKeyURL = "https://blox-key.netlify.app/" -- 🔗 Link key (for normal users)
local scriptURL = "loadstring(game:HttpGet('https://raw.githubusercontent.com/niksscripts/Escape-Tsunami-For-Brainrots/refs/heads/main/loader.lua'))()" -- 🧠 Your main script
...
getKey.Text = "🔗 Get Key (Copied to Clipboard)"
...
-- Clipboard copy for Get Key
getKey.MouseButton1Click:Connect(function()
	if setclipboard then
		setclipboard(linkKeyURL)
		message.Text = "🔗 Link copied to clipboard!"
	else
		message.Text = "⚠️ Executor doesn't support clipboard."
	end
end)
...
submit.MouseButton1Click:Connect(function()
	...
	if keyInput == ownerKey then
		message.Text = "✅ Owner Key Accepted!"
		wait(0.5)
		gui:Destroy()
		...
		if success then
			local validKey = result:match("^%s*(.-)%s*$") -- trim
			...
			message.Text = "⚠️ Couldn’t fetch key. Link broken?"
		end
	end
end)
